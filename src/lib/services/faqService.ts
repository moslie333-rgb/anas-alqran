...
export async function createFAQ(faq: { q: string; a: string; display_order?: number }): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    return { success: false, error: err?.message || "فشل إضافة السؤال" };
  }
}

export async function updateFAQ(id: string, faq: { q: string; a: string; display_order?: number }): Promise<{ success: boolean; error?: string }> {
  if (!isSupabaseConfigured) return { success: true };
  ...
  try {
    ...
    return { success: false, error: err?.message || "فشل تعديل السؤال" };
  }
}
